...
注意！！！此文件夹不能存在""")
    ...
    for j in range (1,13) :
        # 发送请求，获取响
        url = "https://pns.kurogame.com/image/list_2101_" + str (j) + ".html"
        ...
        # d.write (str (mingzi)
        wzlenth = len (pifuwangzhi)
        for k in range (wzlenth) :
            one_link = pifuwangzhi[k]
            one_name = mingzi[k]
            image = requests.get ("https:" + one_link)
            if image.status_code == 200 :
                open (one_name + ".jpg","wb").write (image.content)
